...
if($m->try_post()){
	...
	$con = strip_tags($m->con,'<img><a>');
	preg_match_all($pattern,$con,$match);
	$imgs = $match[0];
	$pics = $match[1];
	$len = count($imgs);
	for($i=0;$i<$len;$i++){
		$iname = explode('.', basename($pics[$i]));
		$iname = $iname[0];
		$wzstr = '';
		if(intval($iname)<105){
			$wzstr = '['.$emotions[$iname].']';
		}		
		$con = str_replace($imgs[$i], $wzstr, $con);
	}
	$m->con = $con;
	$m->save();
	savekw('t', $m);
	Redirect::to('text.html','保存成功');
}
