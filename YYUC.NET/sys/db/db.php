...
class DB{
	 ...
	 * @return DB
	 */
	public static function get_db($conn_name=0){
		if(self::$db === null){
			self::$db = new DB();
			self::$db->to_connect($conn_name);
		}else{
			//特定指定的数据库
			if(is_string($conn_name) && $conn_name!=self::$current_db){
				self::$db->to_connect($conn_name);				
			}elseif($conn_name===true && self::$current_db===null){
				//要切换到从属数据库
				self::$db->to_connect(true);
			}elseif(self::$current_db!=null && $conn_name===null){
				//恢复到默认
				self::$db->to_connect(null);
			}
		}
		return self::$db;
	}
	...
	public function init($host,$port,$username,$password,$dbname){
		...
		if (!$this->conn = @mysql_connect($host.':'.$port, $username, $password)){
			die('<center>DB Connect Error</center>');
		}
		mysql_select_db($dbname, $this->conn);
		mysql_query("SET NAMES ".Conf::$db_charset);
		mysql_query("set charset ".Conf::$db_charset);
	}
	
	/**
	 * 根据配置文件初始化数据库连接类<br/>
	 * 开发中请用get_db方法获取
	 * @param string $conn_name 多库并存的情况下指明要获得连接的名称<br/>传入null主数据库<br/>传入true时随机切换到从属数据库
	 */
	public function to_connect($conn_name=null){
		if(self::$db!==null){
			if(!self::$db->autocommit && self::$db->hascommit===false){
				die('当前数据库事务还未提交或回滚,无法进行库切换，请检查程序源码');
			}
		}
		if($conn_name===null || $conn_name===0){
	 ...
	 * @param array $pam 数字下标的数组，数组项依次替换SQL中的"?"参数
	 */
	public function execute($sql,$pam=null){
		if(is_string($sql)){
	 ...
	 */
	public function query($sql,$pam=null,$callback = null){
		$res = array();
		if(is_string($sql)){
			$sql = str_replace('?', '@yYUc_SqL@', $sql);
		}
	 ...
	 */
	public function query_model($sql,$pam=null,$callback = null){
		$res = array();
		if(is_string($sql)){
			$sql = str_replace('?', '@yYUc_SqL@', $sql);
		}
	 ...
	 * @return array 单列数据集合
	 */
	public function fill_model_one_column($model,$condition=null,$order=null,$limit=null,$field='*',$pam=null){
		$sql = $this->com_sql($model->YYUCSYS_real_tablename,$condition,$order,null,$limit,$field,$pam);
		return $this->one_column_array($sql);
	}
	 ...
	 */
	public function one_column_array($sql,$pam=null){
		$query = $this->base_query($sql,$pam);
		$res = array();
	 ...
	 */
	public function fill_models($model,$condition=null,$order=null,$group=null,$limit=null,$field='*',$pam=null){	
		$sql = $this->com_sql($model->YYUCSYS_real_tablename,$condition,$order,$group,$limit,$field,$pam);
		$res = array();
	 ...
	 * @return array 字符下标的数组集合
	 */
	public function fill_models_array($model,$condition=null,$order=null,$group=null,$limit=null,$field='*',$pam=null){	
		$sql = $this->com_sql($model->YYUCSYS_real_tablename,$condition,$order,$group,$limit,$field,$pam);
	 ...
	 */
	public function fill_one_model(& $model,$condition,$pam=null,$ord=null,$group=null,$limit=null){
		$sql = $this->com_sql($model->YYUCSYS_real_tablename,$condition,$ord,$group,$limit,$model->YYUCSYS_select,$pam);
		$query = $this->base_query($sql);
	 ...
	 */
	public function insert_one_entity($table,$data=array(),$forceinsert = false){		
	 ...
	 */
	public function update_entity($table,$condition=null,$data=array(),$pam=null){
	 ...
	 * @return integer 成功返回受影响的行数，更新失败返回null
	 */
	public function del_entity($table,$condition=null,$pam=null){
	 ...
	 */
	public function has($table,$condition=null,$pam=null){
		$sql = $this->com_sql($table,$condition,null,null,null,'id',$pam);
	 ...
	 */
	public function get_fields_array($table){
	 ...
	 * @param array $pam 数字下标的数组，数组项依次替换$condition中的"?"参数
	 */
	public function com_sql($table,$where=null,$order=null,$group=null,$limit=null,$field='*',$pam=null){
