...
class SampleModel{
	 ...
	 * @return string 标签html字串
	 */
	function time($name,$attrs='',$isdate = true){
		if(is_array($attrs)){
			$attrs = YYUC_get_attrs_from_array($attrs);
		}
		$value = trim($this->$name);
		$datevalue = ($value==''||$value=='0')? '' : $isdate ? date('H:i',strtotime($value)) : date('H:i',$value);
		if($isdate){
			$isdate = 'true';
		}else{
			$isdate = 'false';
		}
		$tag = '<input type="text" '.$this->field_required_string($name).' value="'.$datevalue.'"  id="'.$this->elid($name).'" '.$attrs.' name="'.$this->elname($name).'" relobj="yyuctimepick" rel="yyuc"/>';
		return $tag;
	}
