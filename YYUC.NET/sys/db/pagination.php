...
class Pagination{
	...
	function model_list($model,$back_array = false){
		$db = DB::get_db();
		$sql = $db->com_sql($model->YYUCSYS_real_tablename,$model->YYUCSYS_condition,null,null,null,"count(*) c",$model->YYUCSYS_pam);
	 ...
	 * @param $pam 分页条件
	 */
	function sql_model_list($sql,$pam = null){
		return $this->sql_list($sql,$pam,true);
	}
