...
class Request{
	...
	public static function ip(){
		...
		if(strpos($onlineip, ',')){
			...
			$onlineip = trim($onlineip[0]);
		}
		return $onlineip;
	}
	public static function ex_ip(){
		$ip = 'unknown';
		if($_SERVER["HTTP_ALI_CDN_REAL_IP"] && trim($_SERVER["HTTP_ALI_CDN_REAL_IP"])!='unknown'){
			$ip = $_SERVER["HTTP_ALI_CDN_REAL_IP"];
		}elseif($_SERVER["HTTP_X_FORWARDED_FOR"] && trim($_SERVER["HTTP_X_FORWARDED_FOR"])!='unknown'){
			$ip = $_SERVER["HTTP_X_FORWARDED_FOR"];
		}elseif($_SERVER["HTTP_CLIENT_IP"] && trim($_SERVER["HTTP_CLIENT_IP"])!='unknown'){
			$ip = $_SERVER["HTTP_CLIENT_IP"];
		}elseif ($_SERVER["REMOTE_ADDR"]  && trim($_SERVER["REMOTE_ADDR"])!='unknown'){
			$ip = $_SERVER["REMOTE_ADDR"];
		}elseif ($_SERVER["HTTP_PROXY_USER"]){
