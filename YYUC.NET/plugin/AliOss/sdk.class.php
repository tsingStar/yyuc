...
class ALIOSS{
    ...
    public function __construct($access_id = NULL, $access_key = NULL, $hostname = NULL, $security_token = NULL){
        if($access_id && $access_key){
            ...
            $this->access_id = Conf::$alioss_accesskey;
            $this->access_key = Conf::$alioss_accesssecret;
        }
    ...
    public function upload_file_by_content($object,$content){
    	return self::sys_upload_file_by_content(Conf::$alioss_bucket, $object,array(self::OSS_CONTENT => $content.'',self::OSS_LENGTH => strlen($content)));
    }
    /**
     * 上传文件，适合比较大的文件
     * @throws OSS_Exception
     ...
     * @return ResponseCore
     */
    public function sys_delete_object($bucket, $object, $options = NULL){
        $this->precheck_common($bucket, $object, $options);
        $options[self::OSS_BUCKET] = $bucket;
        $options[self::OSS_METHOD] = self::OSS_HTTP_DELETE;
        $options[self::OSS_OBJECT] = $object;
    ...
    public function delete_object($object, $options = NULL){
    	return self::sys_delete_object(Conf::$alioss_bucket, $object);
    }
    /**
     * 批量删除objects
     * @throws OSS_Exception
     ...
     * @return ResponseCore
     */
    public function sys_get_object($bucket, $object, $options = NULL){
        $this->precheck_common($bucket, $object, $options);
        $options[self::OSS_BUCKET] = $bucket;
        $options[self::OSS_METHOD] = self::OSS_HTTP_GET;
        $options[self::OSS_OBJECT] = $object;
    ...
    public function get_object($object, $options = NULL){
    	return self::sys_get_object(Conf::$alioss_bucket, $object);
    }
