...
function YYUC_autoload($class){
	...
	$file = YYUC_FRAME_PATH.'plugin/'.$class.'.php';
	if (is_file($file)){
		require($file);
		return;
	}
	...
	//图像处理库
	if($class=='ImageWorkshop'){
		require(YYUC_LIB.'plugin/ImageWorkshop/ImageWorkshop.php');
		return;
	}
	//Excel 扩展功能加载
...
if ($_GET['yyuc_sessionid']){
	//记录GET来的sessionid
	session_id(str_replace('@', 'YYUC', $_GET['yyuc_sessionid']));
