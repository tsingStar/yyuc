...
(function ($) {
    ...
    tl.pg.wrapper_markup =
        '<div id="tlyPageGuideWrapper">' +
            ...
            '<div id="tlyPageGuideMessages">' +
                '<a href="#" class="tlypageguide_close" title="Close Guide">关闭</a>' +
    ...
    tl.pg.toggle_markup =
        '<div class="tlypageguide_toggle" title="Launch Page Guide">' +
            ...
            '<a href="#" class="tlypageguide_close" title="close guide">关闭向导 &raquo;</a>' +
        '</div>';
    ...
    tl.pg.init = function(preferences) {
        tl.pgp = preferences = $.extend({}, tl.pg.default_prefs, preferences);
        ...
        if (preferences.custom_open_button == null &&
            ...
            $wrapper.find('#tlyPageGuideToggles').on('mouseup',function(){
            	$('div.subnav > .subnav-title').each(function(){
                	var pul = $(this).parent().find('ul').show();
    ...
    tl.pg.isScrolledIntoView = function(elem) {
    	var poftop = 0;
    	if($(window.tl.pgp.parentel).offset()){
    		poftop = $(window.tl.pgp.parentel).offset().top;
    	}
        var dvtop = $(window.tl.pgp.parentel).scrollTop(),
            dvbtm = dvtop + $(window.tl.pgp.parentel).height(),
            eltop = $(elem).offset().top+dvtop-poftop,
    ...
    tl.pg.PageGuide.prototype.show_message = function (index) {
        ...
        if (target != null) {
            ...
            if (!tl.pg.isScrolledIntoView($(targetKey))) {
            	...
            	var scelel = window.tl.pgp.parentel;
            	if(window.tl.pgp.parentel==window){
            		scelel = 'html,body';
            	}
                $(scelel).animate({scrollTop: (target.targetStyle.top-poftop+$(window.tl.pgp.parentel).scrollTop() - 50)}, 500);
            }
